...
export const receive = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!delivDoc) {
      return next(
        new AppError(
          ...
          [],
          'هذا المنتج غير موجود بالمستودع'
        )
      ) // Handle the case where the delivery document is not found
    }
    ...
    if (storeID != delivDoc.payment.product.store._id) {
      ...
      return next(
        new AppError(
          STATUS_CODE.BAD_REQUEST,
          [],
          `هذا المنتج غير موجود في هذا المستودع يجب الذهاب الى  مستودع ${storeDoc?.name} في العنوان ${storeDoc?.address}`
        )
      )
    }
    ...
    //to check for buyer to take his product
      ...
      if (delivDoc.delivery_status == 'wait') {
        return next(
          new AppError(STATUS_CODE.BAD_REQUEST, [], 'هذا المنتج ليس ضمن مستودع')
        )
      }
      if (delivDoc.delivery_status == 'customer') {
        return next(
          new AppError(
            STATUS_CODE.BAD_REQUEST,
            [],
            'لقد تم تسليمك هذا المنتج رجاء التأكد من رمز التوليد'
          )
        )
      }
      const time: any = new Date()
...
export const generateQrForSeller = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!productDoc || productDoc.user.id != req.user?.id) {
      return next(
        new AppError(STATUS_CODE.BAD_REQUEST, [], 'انت لست مالك هذا المنتج')
      )
    }
...
export const generateQrForCustomer = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const { product } = req.body
    const paymentDoc = (await Payment.find({ product: product }))[0]
    if (!paymentDoc || paymentDoc.customer.id != req.user?.id) {
      return next(
        new AppError(STATUS_CODE.BAD_REQUEST, [], 'قم بشراء المنتج اولا')
      )
    }
    res.status(STATUS_CODE.SUCCESS).json({
      payment: paymentDoc.id,
